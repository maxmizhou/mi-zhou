 ...
 * @author Mi
 */
public abstract class MiBeanCopier {
	private static final Type CONVERTER = TypeUtils.parseType(Converter.class.getName());
	private static final Type BEAN_COPIER = TypeUtils.parseType(MiBeanCopier.class.getName());
	...
	private static final String BEAN_NAME_PREFIX = MiBeanCopier.class.getName();
	/**
	 * The map to store {@link MiBeanCopier} of source type and class type for copy.
	 */
	private static final ConcurrentMap<MiBeanCopierKey, MiBeanCopier> BEAN_COPIER_MAP = new ConcurrentHashMap<>();

	public static MiBeanCopier create(Class source, Class target, boolean useConverter) {
		return MiBeanCopier.create(source, target, useConverter, false);
	}

	public static MiBeanCopier create(Class source, Class target, boolean useConverter, boolean nonNull) {
		MiBeanCopierKey copierKey = new MiBeanCopierKey(source, target, useConverter, nonNull);
		...
		return CollectionUtil.computeIfAbsent(BEAN_COPIER_MAP, copierKey, key -> {
			...
			gen.setContextClass(MiBeanCopier.class);
	...
	public static class Generator extends AbstractClassGenerator {
		private static final Source SOURCE = new Source(MiBeanCopier.class.getName());
		...
		@Override
		public MiBeanCopier create(Object key) {
			return (MiBeanCopier) super.create(key);
		}
